...
export const MessageRow = React.memo(function MessageRow({
  ...
  // One pass per block set: a streaming turn re-renders the whole transcript, and these four
  // derivations used to re-run on every row on every frame.
  const { hasImages, markdown, prose, providerFrame, tools } = useMemo(() => {
    ...
    return {
      ...
      providerFrame: message.blocks.find((block) => block.type === 'text' && block.providerFrame)
    }
  }, [message.blocks])

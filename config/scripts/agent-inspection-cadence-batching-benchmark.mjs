...
// The queue batches tasks enqueued in the same tick; the process-table snapshot TTL then absorbs
// wakes that land within it. Both effects collapse to "distinct wake instants, TTL-bucketed".
...
function simulate(paneCount, baseMs, pickDelay) {
  ...
  for (let pane = 0; pane < paneCount; pane += 1) {
    ...
    while (clock < startedAt + WINDOW_MS) {
      clock += pickDelay(baseMs, clock)
      if (clock >= startedAt + WINDOW_MS) {
        break
      }
      captures.add(Math.floor(clock / SNAPSHOT_TTL_MS))
    }
  }
